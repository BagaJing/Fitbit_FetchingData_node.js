...
var CLIENT_ID = process.env['Fitbit_Client'];
var CLIENT_SECRET = process.env['Fitbit_password'];
...
setTimeout( function () {
      db.collection('history').insertOne( json, function(err, result) {
      if(err!=null) {
          clog.error("an error occurred in createDoc", err);
          callback(null, JSON.stringify(err));
      }
      else {
        clog.log(" You just created an record!");
...
db.collection('profiles').find({},(err,profiles)=>{
...
var FitbitSync = function(subscr,db){
        ...
        if (subscriber.userID=='test') {
            ...
            if ((result[0].errors && result[0].errors.length > 0 && result[0].errors[0].errorType === 'expired_token')){
                ...
                fitbit.refreshAccessToken(subscriber.access_token,subscriber.refresh_token,28800).then(function(newtoken){
                    ...
                    db.collection('accessbackups').findOne({userID:subscriber.userID},function(err,doc){
                        if (doc) {
    ...
    var _updateDatabase = function(subscriber){
        ...
        if (needUpdate) {
            db.collection('profiles').updateOne({userID:subscriber.userID},{$set: {
    ...
    var _updateTokens = function(subscriber,Backup){
        ...
        if(needFresh){
             db.collection('profiles').updateOne({userID:subscriber.userID},{$set:{access_token: subscriber.access_token,
            ...
            db.collection('accessbackups').updateOne({userID:subscriber.userID},{$set: {access_token: subscriber.access_token,
                                                               refresh_token: subscriber.refresh_token}},
                function(err,docs){
                if (err) {clog.error(err);}
            });
